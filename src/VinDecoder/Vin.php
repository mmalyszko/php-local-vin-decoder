...
class Vin
{
    ...
    /**
     * IMPORTANT: Use only one VDS file per process.
     * VDS data is cached per class, so changing the file affects all Vin instances.
     */
    ...
    public function decodeModel(): ?string
    {
        ...
        // In North America & China 9th char is checksum and varies even for the same model, so wildcard can be used
        $vds5 = substr($this->vds, 0, -1).'*';

        $model = self::$vmiCached[$this->wmi][$this->vds] ?? self::$vmiCached[$this->wmi][$vds5] ?? null;
        ...
        return self::$vmiCached[$wmi2][$this->vds] ?? self::$vmiCached[$wmi2][$vds5] ?? null;
    }
